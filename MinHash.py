...
for i in range(0, numDocs):
  ...
  # Maintain a list of document IDs (article IDs)
    readWords = f.readline().split(" ")
    documentNames.append(readWords[0])
# Create a set of shingles called shinglesInDocument
  ...
  # automatically remove duplicate shingles.
    for i in range(0, len(readWords))
        shingle = readWords[i] + readWords[i+1] + readWords[i+2]
  ...
  # Add the hashed value to the set.
        ShingleToInt = binascii.crc32(shingle)
        shinglesInDocument.add(ShingleToInt)

  # Now add the completed set of shingles to the documentShingleSets dictionary.
    documentShingleSets[readWords[0]] = shinglesInDocument
  # Keep a count of the total number of shingles found in numShingles.
    numShingles = numShingles + len(readWords)
# Close the data file.  
f.close()  
